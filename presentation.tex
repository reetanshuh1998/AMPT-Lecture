...
\begin{frame}{Why Experimental High Energy Physics? Probing the Extremes}
    ...
    \begin{alertblock}{Interactive Physics Simulator}
        ...
        \begin{itemize}
            \item \texttt{animations/04\_qcd\_confinement\_screening.html} \quad {\tiny (QCD flux-tube snapping, Debye screening, \& Matsui-Satz quarkonium melting)}
        \end{itemize}
    \end{alertblock}
\end{frame}
...
\begin{frame}{Hyperbolic Mappings to Rapidity}
    ...
    \end{block}
    \begin{block}{Physical Intuition: Spacetime Hyperbolas}
        In Minkowski space, constant transverse mass curves are hyperbolas ($E^2 - p_z^2 = m_T^2$). Rapidity acts as the hyperbolic angle that rotates coordinates under longitudinal Lorentz boosts, preserving the shape of rapidity yields.
    \end{block}
\end{frame}

% Slide 27: From Rapidity to Pseudorapidity
\begin{frame}{From Rapidity to Pseudorapidity}
